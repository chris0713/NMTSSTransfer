...
namespace NMTSSTransfer
{
    class CalTransfer
    {
       ...
       /**
        ...
        public void getCVS(string uid , string pwd , string[] links)
        {
            ...
            string header = @"""主旨"",""開始日期"",""開始時間"",""結束日期"",""結束時間"",""全天"",""提醒開啟/關閉"",""提醒日期"",""提醒時間"",""會議召集人"",""出席者"",""列席者"",""會議資源"",""地點"",""忙閒狀態"",""私人"",""津貼"",""帳目資訊"",""敏感度"",""描述"",""優先順序"",""類別"" ";
                ...
                try
                {
                    using (StreamWriter sw = new StreamWriter(@"c:\Calendar.csv", false, Encoding.GetEncoding(950)))
                    {
        ...
        private string getMeetingInfo(string htmlCode)
        {
            ...
            strAddress = strAddress.Replace("<br />", "-").Replace("\r", "").Replace("\n", "").Trim();
        ...
        private string composeCanlenderText(string strSubject, string strDate, string strStartTime, string strEndTime, string strStackholder, string strPhoneNum, string strAddress, string strComments, string strSpeaker)
        {
            
            //string contents = @""主旨,"2009/5/14","上午 09:00:00","2009/5/14","上午 09:30:00","假","假","2009/5/14","上午 08:45:00",,,,,"地點","2","假",,,"普通","comments","普通"

            StringBuilder sb = new StringBuilder();
            sb.Append("\"").Append(strSpeaker).Append("-").Append(strSubject).Append("\",")
              .Append("\"").Append(strDate).Append("\",")
              .Append("\"").Append(strStartTime).Append("\",")
              .Append("\"").Append(strDate).Append("\",")
              .Append("\"").Append(strEndTime).Append("\",")
              .Append("\"假\",\"假\"").Append(",")
              .Append("\"").Append(strDate).Append("\",")
              .Append("\"").Append(strStartTime).Append("\"").Append(@",,,,,")
        ...
        private static string getWords(ref string htmlCode, string keyword, bool flag)
        {
            try
            {
                string targetString = string.Empty;
